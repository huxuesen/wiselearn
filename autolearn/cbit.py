...
# 直接 print 输出日志，确保 Docker 中能看到
def _log(msg: str):
    print(f"[cbit] {msg}", flush=True)
...
class CbitPlatform(BasePlatform):
    def __init__(self, user_info: Dict[str, str], progress_callback=None) -> None:
        ...
        _log(f"初始化刷课任务: phone={self.phone}, tcid={self.lesson_library_id}")

    async def _report(self, msg: str, progress: int = 0):
        """报告进度"""
        _log(f"[{self.phone}] {msg} ({progress}%)")
    ...
    async def _run_ocr(self, image_bytes: bytes) -> str:
        ...
        if not captcha_text:
            _log(f"[{self.phone}] OCR 识别失败")
            raise CaptchaError("OCR failed to recognize captcha")
        _log(f"[{self.phone}] OCR 识别成功: {captcha_text}")
        return captcha_text
    ...
    async def _post_schedule(self, lesson_id: str, item_id: str, total_time: float, lesson_name: str, tcid: str, study_plan: float = 0) -> None:
        ...
        if self.mode == "fast":
            ...
            for attempt in range(self.retry):
                ...
                result: dict = resp.json()
                if result.get("success"):
                    _log(f"[{self.phone}] 课时上报成功: {lesson_name} ({total_time}s) tcid={tcid}")
                    return
                else:
                    _log(f"[{self.phone}] 课时上报失败 (第{attempt+1}次): {lesson_name} - {result}")
    ...
    async def learn(self) -> None:
        ...
        for lesson_id in lesson_ids:
            ...
            if not lesson_items:
                _log(f"[{self.phone}] 课程 {lesson_id} 无学习项，跳过")
                continue

            _log(f"[{self.phone}] 课程 {lesson_id}: {len(lesson_items)} 个学习项")

            for item in lesson_items:
                _log(f"[{self.phone}] 上报课时: {item['name']} ({item['time']}s)")
